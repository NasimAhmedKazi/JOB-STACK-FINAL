...
const FindJobPage = () => {
  ...
  const toggleSaveJob = async (jobId) => {
    try {
      ...
      const res = await fetch(`http://localhost:8000/api/saved/job/${jobId}`, {
        method: "POST",
        headers: {
  ...
  const confirmApply = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8000/api/application/apply/${confirmToast.jobId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    const fetchAppliedJobs = async () => {
      try {
        ...
        const res = await fetch("http://localhost:8000/api/application/user", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
    ...
    const fetchSavedJobs = async () => {
      try {
        ...
        const res = await fetch("http://localhost:8000/api/saved", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8000/api/job?${params.toString()}`,
      );
      const data = await res.json();
          ...
          if (job.companyLogo) {
            if (job.companyLogo.startsWith("http")) {
              ...
              const path = job.companyLogo.startsWith("/")
                ? job.companyLogo
                : `/${job.companyLogo}`;
              logoSrc = `http://localhost:8000${path
                .split("/")
  ...
  const renderLogo = (job) => {
    ...
    const isBroken =
      imgErrors[job.id] || !logoUrl || logoUrl === "http://localhost:8000";
